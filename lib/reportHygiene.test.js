...
test('stripRawText drops top-level rawText (the full PDF text)', () => {
  const input = {
    rawText: 'A'.repeat(500_000), // simulate 500KB PDF text
    reportNo: 'R1',
    facilities: [{ contractCode: 'A', rawText: 'pdf' }],
  };
  const out = stripRawText(input);
  assert.equal(out.rawText, undefined);
  assert.equal(out.reportNo, 'R1');
  assert.equal(out.facilities[0].rawText, undefined);
  // Input unchanged
  assert.equal(input.rawText.length, 500_000);
});

test('stripForPrint drops top-level rawText AND per-facility rawText/history', () => {
  const input = {
    rawText: 'full pdf text',
    reportNo: 'R1',
    facilities: [{ contractCode: 'A', rawText: 'x', history: [{ m: 1 }] }],
  };
  const out = stripForPrint(input);
  assert.equal(out.rawText, undefined);
  assert.equal(out.reportNo, 'R1');
  ...
  assert.equal(out.facilities[0].history, undefined);
  // Input unchanged
  assert.equal(input.rawText, 'full pdf text');
});
