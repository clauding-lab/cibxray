...
test('MANUAL_SLIDES: tab-tour slides show screenshot placeholders', () => {
  const tabTourSlides = MANUAL_SLIDES.filter(s =>
    s.kind === 'content' && s.title.startsWith('Tab tour'));
  // At least the major-tab slides should have placeholders for banker to paste screenshots
  assert.ok(tabTourSlides.length >= 5, `Expected at least 5 tab-tour slides, got ${tabTourSlides.length}`);
  const withPlaceholder = tabTourSlides.filter(s => s.showsPlaceholder);
  assert.ok(withPlaceholder.length >= 5,
    `Expected at least 5 tab-tour slides with screenshot placeholders, got ${withPlaceholder.length}`);
});
