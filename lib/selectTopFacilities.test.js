...
test('tiebreak works with parser-emitted title-case status ("Live" / "Historical")', () => {
  const input = [
    { outstanding: 100, status: 'Historical', nature: 'Funded', id: 'h' },
    { outstanding: 100, status: 'Live', nature: 'Funded', id: 'l' },
  ];
  const out = selectTopFacilities(input, 10);
  assert.equal(out[0].id, 'l');
  assert.equal(out[1].id, 'h');
...
test('tiebreak works with parser-emitted title-case nature ("Funded" / "Non-Funded")', () => {
  const input = [
    { outstanding: 100, status: 'Live', nature: 'Non-Funded', id: 'nf' },
    { outstanding: 100, status: 'Live', nature: 'Funded', id: 'f' },
  ];
  const out = selectTopFacilities(input, 10);
  assert.equal(out[0].id, 'f');
  assert.equal(out[1].id, 'nf');
