...
test('buildTermLoansRows emits Term/Paid/Remaining term correctly', () => {
  const rows = buildTermLoansRows(report([
    fac({
      contractCode: 'COUNT',
  ...
  // Per spec: Term | Due | Paid | Remaining term
  assert.equal(dataRow[11], 60); // Term
  // Due — judgment call per spec (unclear derivation); renders empty string
  assert.equal(dataRow[12], '');
