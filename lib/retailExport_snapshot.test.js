...
test('retail workbook shape matches snapshot fixture', () => {
  ...
  // Act — pass the fixture's asOf to stabilize time-sensitive cells (Sheet 3
  // Due EMI). The top-level Findings row still uses `new Date()` internally,
  // so we stabilize "Age of CIB Report (Days)" afterward.
  const actual = stabilizeAgeDays(buildRetailWorkbookData(retailSampleReport, { asOf }));
