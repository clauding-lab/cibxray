...
test('computeDueEmi: monthly loan, 2 years in → 24 due', () => {
  const f = fac({
    startDate: '01/01/2024', endDate: '01/01/2029',
    paymentPeriodicity: 'Monthly', totalInstallments: 60,
  });
  const asOf = new Date('2026-01-01');
  assert.equal(computeDueEmi(f, asOf), 24);
});

test('computeDueEmi: quarterly loan, 9 months in → 3 due', () => {
  const f = fac({
    startDate: '01/01/2024', endDate: '01/01/2027',
    paymentPeriodicity: 'Quarterly', totalInstallments: 12,
  });
  const asOf = new Date('2024-10-01');
  assert.equal(computeDueEmi(f, asOf), 3);
});

test('computeDueEmi: half-yearly loan, 12 months in → 2 due', () => {
  const f = fac({
    startDate: '01/01/2024', endDate: '01/01/2027',
    paymentPeriodicity: 'Half-Yearly', totalInstallments: 6,
  });
  const asOf = new Date('2025-01-01');
  assert.equal(computeDueEmi(f, asOf), 2);
});

test('computeDueEmi: bimonthly loan, 6 months in → 3 due', () => {
  const f = fac({
    startDate: '01/01/2024',
    paymentPeriodicity: 'Bimonthly', totalInstallments: 12,
  });
  const asOf = new Date('2024-07-01');
  assert.equal(computeDueEmi(f, asOf), 3);
});

test('computeDueEmi: caps at totalInstallments (matured loan)', () => {
  const f = fac({
    startDate: '01/01/2020',
    paymentPeriodicity: 'Monthly', totalInstallments: 36,
  });
  const asOf = new Date('2026-01-01'); // 72 months elapsed
  assert.equal(computeDueEmi(f, asOf), 36);
});

test('computeDueEmi: asOf before startDate → 0', () => {
  const f = fac({
    startDate: '01/06/2026',
    paymentPeriodicity: 'Monthly', totalInstallments: 36,
  });
  const asOf = new Date('2026-01-01');
  assert.equal(computeDueEmi(f, asOf), 0);
});

test("computeDueEmi: missing startDate → ''", () => {
  const f = fac({ startDate: '', paymentPeriodicity: 'Monthly' });
  const asOf = new Date('2026-01-01');
  assert.equal(computeDueEmi(f, asOf), '');
});

test("computeDueEmi: missing paymentPeriodicity → ''", () => {
  const f = fac({ startDate: '01/01/2024', paymentPeriodicity: '' });
  const asOf = new Date('2026-01-01');
  assert.equal(computeDueEmi(f, asOf), '');
});

test('computeDueEmi: no totalInstallments cap → pure elapsed count', () => {
  const f = fac({
    startDate: '01/01/2024', paymentPeriodicity: 'Monthly',
    totalInstallments: null,
  });
  const asOf = new Date('2025-01-01');
  assert.equal(computeDueEmi(f, asOf), 12);
});
...
// Bug 1 — underscore suffix treated as same group
test('detectApplyingConcern: underscore suffix → same group, no throw', () => {
  // Arrange
  const base = { ...rep([]), fileName: '1011500041801.pdf' };
  const sister = { ...rep([]), fileName: '1011500041801_1.pdf' };
  // Act
  const result = detectApplyingConcern([base, sister]);
  // Assert
  assert.equal(result.applyingConcern.fileName, '1011500041801.pdf');
  assert.equal(result.sisterConcerns.length, 1);
  assert.equal(result.groupRefBase, '1011500041801');
});

// Bug 1 — descriptive prefix before digits
test('detectApplyingConcern: descriptive prefix → same group, no throw', () => {
  // Arrange
  const base = { ...rep([]), fileName: '1011500041801.pdf' };
  const sister = { ...rep([]), fileName: 'Acme 1011500041801-1.pdf' };
  // Act
  const result = detectApplyingConcern([base, sister]);
  // Assert
  assert.equal(result.applyingConcern.fileName, '1011500041801.pdf');
...
// Bug 1 — parentheses around ref number
test('detectApplyingConcern: parentheses around ref → same group, no throw', () => {
  // Arrange
  const base = { ...rep([]), fileName: '(1011500041801).pdf' };
  const sister = { ...rep([]), fileName: '1011500041801-1.pdf' };
  // Act
  const result = detectApplyingConcern([base, sister]);
  // Assert
  assert.equal(result.sisterConcerns.length, 1);
...
// Bug 1 — double extension
test('detectApplyingConcern: double extension → applying concern with clean refBase', () => {
  // Arrange
  const r = { ...rep([]), fileName: '1011500041801.PDF.pdf' };
  // Act
  const result = detectApplyingConcern([r]);
  // Assert
  assert.equal(result.applyingConcern.fileName, '1011500041801.PDF.pdf');
  assert.equal(result.groupRefBase, '1011500041801');
});

// Bug 2 — rogue suffix file from different base mixed in
test('detectApplyingConcern: rogue suffix file with different base → throws', () => {
  // Arrange
  const r1 = { ...rep([]), fileName: '1011500041801.pdf' };
  const r2 = { ...rep([]), fileName: '1011500041801-1.pdf' };
  const r3 = { ...rep([]), fileName: '2099887766554-1.pdf' };
  // Act & Assert
  assert.throws(() => detectApplyingConcern([r1, r2, r3]), /multiple|ambiguous/i);
});

// Bug 2 — two suffix files from different bases, no bare file
test('detectApplyingConcern: two suffix-only files, different bases → throws', () => {
  // Arrange
  const r1 = { ...rep([]), fileName: '1011500041801-1.pdf' };
  const r2 = { ...rep([]), fileName: '2099887766554-1.pdf' };
  // Act & Assert
  assert.throws(() => detectApplyingConcern([r1, r2]), /multiple|ambiguous/i);
});

// err.bases shape — structured data for the UI formatter
test('detectApplyingConcern: multi-base throw carries err.bases with per-base counts ordered by desc count', () => {
  // Arrange — 5 files for base A, 1 file for base B
  const mkFile = (name) => ({ ...rep([]), fileName: name });
  const files = [
    mkFile('1011500041801.pdf'),
    mkFile('1011500041801-1.pdf'),
    mkFile('1011500041801-2.pdf'),
    mkFile('1011500041801-3.pdf'),
    mkFile('1011500041801-4.pdf'),
    mkFile('2099887766554.pdf'),
  ];
  // Act
  let caughtErr;
  try {
    detectApplyingConcern(files);
  } catch (e) {
    caughtErr = e;
  }
  // Assert
  assert.ok(caughtErr, 'should have thrown');
  assert.ok(Array.isArray(caughtErr.bases), 'err.bases must be an array');
  assert.equal(caughtErr.bases.length, 2, 'exactly 2 base entries');
  // First entry is the one with the higher count
  assert.equal(caughtErr.bases[0].base, '1011500041801');
  assert.equal(caughtErr.bases[0].count, 5);
  // Second entry is the one with the lower count
  assert.equal(caughtErr.bases[1].base, '2099887766554');
  assert.equal(caughtErr.bases[1].count, 1);
});

// ── Short-prefix fix (fix/multi-base-short-prefix) ──────────────────────────
// Tier-2 longest-common-prefix fallback: handles filenames whose digit run is
// shorter than 10 chars and would otherwise each become their own ref-base.

// Helper: produce an array of report objects for a hyphenated short base with
// N suffix files plus an optional bare base file.
function makeHyphenatedSet(base, suffixCount) {
  const files = [{ ...rep([]), fileName: `${base}.pdf` }];
  for (let i = 1; i <= suffixCount; i++) {
    files.push({ ...rep([]), fileName: `${base}-${i}.pdf` });
  }
  return files;
}

// Same helper but non-hyphenated (no separator in base itself).
function makeNonHyphenatedSet(base, suffixCount) {
  return makeHyphenatedSet(base, suffixCount); // identical structure, different base string
}

// Test 1 — Hyphenated short base, full set (bare + 14 suffix files)
test('detectApplyingConcern [short-prefix]: hyphenated short base full set → single group', () => {
  // Arrange
  const files = makeHyphenatedSet('0426-0427', 14);
  // Act
  const result = detectApplyingConcern(files);
  // Assert
  assert.equal(result.groupRefBase, '0426-0427');
  assert.equal(result.applyingConcern.fileName, '0426-0427.pdf');
  assert.equal(result.sisterConcerns.length, 14);
});

// Test 2 — Non-hyphenated short base (8-digit), full set (bare + 14 suffix files)
test('detectApplyingConcern [short-prefix]: non-hyphenated 8-digit base full set → single group', () => {
  // Arrange
  const files = makeNonHyphenatedSet('04260427', 14);
  // Act
  const result = detectApplyingConcern(files);
  // Assert
  assert.equal(result.groupRefBase, '04260427');
  assert.equal(result.applyingConcern.fileName, '04260427.pdf');
  assert.equal(result.sisterConcerns.length, 14);
});

// Test 3 — Short base, no bare file → fallback to smallest suffix
test('detectApplyingConcern [short-prefix]: short base no bare file → smallest suffix is applying', () => {
  // Arrange
  const r1 = { ...rep([]), fileName: '04260427-1.pdf' };
  const r2 = { ...rep([]), fileName: '04260427-2.pdf' };
  // Act
  const result = detectApplyingConcern([r1, r2]);
  // Assert
  assert.equal(result.groupRefBase, '04260427');
  assert.equal(result.applyingConcern.fileName, '04260427-1.pdf');
});

// Test 4 — LCP over-match must trim back
// Raw LCP of ['04260427-1', '04260427-10', '04260427-11'] = '04260427-1'
// Tails would be ['', '0', '1'] — '0' and '1' don't match ^[-_]\d+$ → invalid.
// Iterative trim should peel back to '04260427' with tails ['-1', '-10', '-11'].
test('detectApplyingConcern [short-prefix]: LCP over-match trims back to valid prefix', () => {
  // Arrange
  const r1 = { ...rep([]), fileName: '04260427-1.pdf' };
  const r2 = { ...rep([]), fileName: '04260427-10.pdf' };
  const r3 = { ...rep([]), fileName: '04260427-11.pdf' };
  // Act
  const result = detectApplyingConcern([r1, r2, r3]);
  // Assert
  assert.equal(result.groupRefBase, '04260427');
  assert.equal(result.applyingConcern.fileName, '04260427-1.pdf');
  assert.equal(result.sisterConcerns.length, 2);
});

// Test 5 — Mixed schemes with no shared prefix → still throws multi-base error
test('detectApplyingConcern [short-prefix]: truly different bases → throws multi-base error', () => {
  // Arrange: a 13-digit Tier-1 base and an 8-digit short base share no common prefix
  const r1 = { ...rep([]), fileName: '1011500041801.pdf' };
  const r2 = { ...rep([]), fileName: '04260427-1.pdf' };
  // Act & Assert
  assert.throws(() => detectApplyingConcern([r1, r2]), /multiple|ambiguous/i);
});

// Test 6 — Single-file short base
test('detectApplyingConcern [short-prefix]: single short-base file → applying, no sisters', () => {
  // Arrange
  const r = { ...rep([]), fileName: '04260427.pdf' };
  // Act
  const result = detectApplyingConcern([r]);
  // Assert
  assert.equal(result.groupRefBase, '04260427');
...
// Test 7 — Underscore suffix separator on short base
test('detectApplyingConcern [short-prefix]: underscore suffix separator → same group', () => {
  // Arrange
  const bare = { ...rep([]), fileName: '04260427.pdf' };
  const s1 = { ...rep([]), fileName: '04260427_1.pdf' };
  const s2 = { ...rep([]), fileName: '04260427_2.pdf' };
  // Act
  const result = detectApplyingConcern([bare, s1, s2]);
  // Assert
  assert.equal(result.groupRefBase, '04260427');
  assert.equal(result.applyingConcern.fileName, '04260427.pdf');
  assert.equal(result.sisterConcerns.length, 2);
...
// Test 8 — Hyphenated short base with mixed separator types (- and _)
test('detectApplyingConcern [short-prefix]: hyphenated base mixed separators → single group', () => {
  // Arrange
  const bare  = { ...rep([]), fileName: '0426-0427.pdf' };
  const dash  = { ...rep([]), fileName: '0426-0427-1.pdf' };
  const under = { ...rep([]), fileName: '0426-0427_2.pdf' };
  // Act
  const result = detectApplyingConcern([bare, dash, under]);
  // Assert
  assert.equal(result.groupRefBase, '0426-0427');
  assert.equal(result.applyingConcern.fileName, '0426-0427.pdf');
  assert.equal(result.sisterConcerns.length, 2);
